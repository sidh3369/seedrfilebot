...
export default {
    ...
    async scheduled(_event, env, _ctx) {
        ...
        try {
            ...
            const { results: users } = await env.DB.prepare('SELECT DISTINCT telegram_user_id FROM users WHERE id IN (SELECT user_id FROM seedr_accounts WHERE is_active = 1)').all();
            ...
            for (const user of users) {
                try {
                    // Get user's accounts
                    const { results: accounts } = await env.DB.prepare('SELECT * FROM seedr_accounts WHERE user_id = (SELECT id FROM users WHERE telegram_user_id = ?) AND is_active = 1').bind(user.telegram_user_id).all();
