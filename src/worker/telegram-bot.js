...
export async function setupBotCommands(env) {
...
export function createBot(env) {
    ...
    bot.command('start', async (ctx) => {
        ...
        await ctx.reply('🚀 *Welcome to SeedrSync Bot!*\n\n' +
            ...
            '_Made by Shailesh Patel_', { parse_mode: 'Markdown' });
    });
    bot.command('accounts', async (ctx) => {
        ...
        for (const account of accounts) {
            ...
            await ctx.reply(`📧 ${email}\nAdded: ${addedDate}`, { reply_markup: keyboard });
        }
    });
    bot.command('m3u', async (ctx) => {
        try {
            ...
            const accounts = await getUserSeedrAccounts(env, user.id);
            if (accounts.length === 0) {
                await ctx.api.editMessageText(ctx.chat.id, statusMsg.message_id, '❌ No Seedr accounts connected.\n\nUse /login2 to add your account.');
                return;
            }
            ...
            if (totalVideos === 0) {
                await ctx.api.editMessageText(ctx.chat.id, statusMsg.message_id, '❌ No video files found in your accounts.');
                return;
            }
            const githubUrl = await uploadM3U8ToGitHub(env, m3u8Content, `user-${telegramUserId}.m3u8`);
            await ctx.api.editMessageText(ctx.chat.id, statusMsg.message_id, `✅ *Your Personal M3U8 Playlist Generated!*\n\n` +
                `🎬 Total Videos: ${totalVideos}\n` +
                `🔗 [Download M3U8](${githubUrl})\n\n` +
                `_This playlist includes only your videos_`, { parse_mode: 'Markdown' });
        }
        catch (error) {
    ...
    bot.command('login2', async (ctx) => {
        ...
        await ctx.reply('🔐 *Add Seedr Account*\n\n' +
            'Please send your Seedr email address:', { parse_mode: 'Markdown' });
    });
    bot.command('help', async (ctx) => {
        await ctx.reply('*📚 SeedrSync Bot Help*\n\n' +
            '*Commands:*\n' +
                ...
                try {
                    ...
                    if (folders.length > 0) {
                        ...
                        for (const folder of folders.slice(0, 10)) {
                            ...
                            await ctx.reply(`📂 *${folder.name}*${size}`, { parse_mode: 'Markdown', reply_markup: keyboard });
                            totalFolders++;
                        }
    ...
    bot.on('message:text', async (ctx) => {
        ...
        try {
            ...
            if (folder) {
                ...
                await ctx.reply('✅ *Magnet link added successfully!*\n\n' +
                    '_Processing... Use /files to view when ready._', { parse_mode: 'Markdown' });
            }
        }
        catch (error) {
            await ctx.reply(`❌ Error: ${error.message}`);
...
export function createWebhookHandler(env) {
